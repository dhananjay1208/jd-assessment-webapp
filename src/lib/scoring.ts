...
/** A short verdict line for the result screen. */
export function verdictLine(pct: number): string {
  if (pct >= 80) return 'Strong. You have a solid grip on both blocks.'
  if (pct >= 60) return 'A pass. A few topics are worth a second look.'
  return 'Below the pass mark. Revise the apps and read the review below.'
}
